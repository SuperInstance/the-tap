...
| Repo | Verdict | Brief |
|---|---|---|
| [mud-arena](mud-arena.md) | Two disconnected codebases: a real, 303-test Python perceive-decide-act package, plus five untested/non-functional "simulation" scripts (server.py can't even run) and unbuildable `.cu`/`.zig`/`.c` files that are raw unstripped LLM output | mud-arena.md |
...
| [ternary-tenforward](ternary-tenforward.md) | Real, tested 602-line Rust conversation-dynamics engine. Used as the model for tap-dynamics — comparison found and fixed one bug (see below) | ternary-tenforward.md |
...
does — mud-arena's CLI flags and evolution engine, git-native-mud's "zero server," ec2mud's "EC2,"
crab-trap-web's room count, ternary-tenforward's "Mutation (5%)/Energy decay/Trust realignment."
...
The ternary-tenforward brief directly compared its findings against `tap-dynamics` (written earlier
in this same build, from a verbal description, without reading ternary-tenforward's source) and
...
`Speaker`s never affect each other, whereas ternary-tenforward's entire dynamic comes from O(n²)
pairwise `react_to` each round plus `energy`/`trust`/`dominance` gating. See ternary-tenforward.md §3
...
1. **Event bus + append-only log** (mud-arena's `EventBus`) — tap-room currently mutates state with
...
2. **Command/Verb text-parsing layer** (mud-arena's `Command`/`parse_command`) — a shape for turning
   free text or LLM output into tap-room's `Action` enum, which currently has no text-command layer
   above it.
3. **Multi-speaker interaction model** (ternary-tenforward's pairwise `react_to` + energy/trust/
   dominance) — the actual missing piece in tap-dynamics right now.
4. **What to actively avoid:** mud-arena's disconnected GA/DSL split (don't build a generic evolution
